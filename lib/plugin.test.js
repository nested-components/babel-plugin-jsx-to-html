...
function example(name, input) {
  it(name, () => {
    const { code } = (0, _babelCore.transform)(input, {
      babelrc: false,
...
describe("plugin", () => {
  ...
  example("Nested Component with attributes", `
      import hyperHTML from "hyperhtml";
  ...
  example("Custom Components", `
    ...
    `);
  example("Components with multiple children", `
      import hyperHTML from "hyperhtml";
      const Greet = ({name}) => <h1 class="yo"><span>Hello {name}</span><Icon name="+1" /></h1>
    `);
  example("Custom Components with a single child", `
      import hyperHTML from "hyperhtml";
      const Greet = ({name}) => <Foo><Bar /></Foo>
    `);
  example("Custom Components with multiple children", `
      import hyperHTML from "hyperhtml";
      const Greet = ({name}) => <Foo><Bar /><Baz /></Foo>
    `);
  example("Custom Components with mixed children", `
      import hyperHTML from "hyperhtml";
  ...
  example("Multiple components", `
    ...
    `);
  example("Import specifier", `
      import {wire} from "hyperhtml";
      const Greet = ({name}) => <span>Hello {name}</span>
    `);
  example("Import namespace specifier", `
      import * as h from "hyperhtml";
      const Greet = ({name}) => <span>Hello {name}</span>
    `);
  example("Array Pattern as first argument", `
      import * as h from "hyperhtml";
      const Greet = ([name]) => <span>Hello {name}</span>
    `);

  example("Normal var as first argument", `
      import * as h from "hyperhtml";
      const Greet = (props) => <span>Hello {props.name}</span>
    `);

  example("HyperComponent", `
  ...
  example("HyperComponent with imported namespace", `
    import * as H from "hyperhtml";
  ...
  example("Ignore file with no imports", `
